...
export function FocusProvider({ children }) {
  ...
  const pushFocus = (focus) => {
    setFocusStack((prevStack) => {
      ...
      const newStack =
        lastValidIndex !== -1 ? prevStack.slice(0, lastValidIndex) : prevStack;
  ...
  const buildParentChain = (stack, index) => {
    if (index <= 0) return null; // Base case: no parent
    const parent = stack[index - 1];
    return { ...parent, parent: buildParentChain(stack, index - 1) }; // Recursive parent chain
  };

  const currentFocus = {
    ...focusStack[focusStack.length - 1],
    parent: buildParentChain(focusStack, focusStack.length - 1), // Add recursive parent chain
  };
